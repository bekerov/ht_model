...
if __name__=='__main__':
    possible_states, possible_start_states, possible_state_actions = ts.load_states()
    pi_1 = sf.init_random_policy(possible_state_actions)
    pi_2 = sf.init_random_policy(possible_state_actions)
    state_r1 = random.choice(tuple(possible_start_states))
    state_r2 = state_r1
    ...
    while True:
        ...
        if state_r1 not in policy:
            action_r1 = random.choice(tuple(possible_state_actions[state_r1]))
        else:
            action_r1 = sf.softmax_select_action(policy[state_r1])
        if state_r2 not in policy:
            action_r2 = random.choice(tuple(possible_state_actions[state_r2]))
        else:
            action_r2 = sf.softmax_select_action(policy[state_r2])
