...
def simulate_next_state(current_action, my_current_state, teammate_current_state):
    ...
    if current_action == 'TR':
        ...
        my_next_state = ts.State(my_current_state.n_r-1, my_current_state.n_h, my_current_state.t_r, my_current_state.t_h, 1, my_current_state.b_h, my_current_state.e)
    if current_action == 'TH':
        ...
        my_next_state = ts.State(my_current_state.n_r, my_current_state.n_h-1, my_current_state.t_r, my_current_state.t_h, my_current_state.b_r, 1, my_current_state.e)
    if current_action == 'K':
        # current_action is keep box on table, so only my_current_state changes
        # setting robot holding its box to 0
        my_next_state = ts.State(my_current_state.n_r, my_current_state.n_h, my_current_state.t_r, my_current_state.t_h, 0, my_current_state.b_h, my_current_state.e)
    if current_action == 'G':
    ...
    if current_action == 'R':
        ...
        # robot is receving
        my_next_state = ts.State(my_current_state.n_r, my_current_state.n_h, my_current_state.t_r, 0, 1, my_current_state.b_h, my_current_state.e)
        teammate_next_state = ts.State(teammate_current_state.n_r, teammate_current_state.n_h, 0, teammate_current_state.t_h, teammate_current_state.b_r, 0, teammate_current_state.e)
